...
def transcribe_video(video_path, model_size="base", output_format="txt"):
    ...
    try:
        # 文字起こし実行 (fp16=False はCPU実行時の警告抑制のため)
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("使用方法: python transcribe_cpu.py <動画ファイルのパス> [モデルサイズ]")
        print("例: python transcribe_cpu.py my_video.mp4 small")
    else:
